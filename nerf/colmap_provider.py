...
def center_poses(poses, pts3d=None, enable_cam_center=False,offset = [0,0,0]):
...
class ColmapDataset:
    def __init__(self, opt, device, type='train', n_test=3):
        super().__init__()
        ...
        candidate_paths = [
            ...
            os.path.join(self.root_path, "colmap","sparse", "0"),
        ]
        ...
        if os.path.exists(os.path.join(self.root_path, f"images_{self.downscale}")):
            img_folder = os.path.join(self.root_path, f"images_{self.downscale}")
        elif os.path.exists(os.path.join(self.root_path, "rgb")):
            img_folder = os.path.join(self.root_path, "rgb")
        elif os.path.exists(os.path.join(self.root_path, "images")):
            img_folder = os.path.join(self.root_path, "images")

        img_paths = np.array([os.path.join(img_folder, name.replace('JPG','jpg')) for name in img_names])
        if self.opt.temp:
            img_paths = np.array([os.path.join(self.root_path, "warped_thermal", name.replace('JPG','jpg')) for name in img_names])
        
        if self.opt.ts:
            img_paths = np.array([os.path.join(self.root_path, "warped_thermal", name[:-4]+'.npy') for name in img_names])
        
        if self.opt.rgbt or self.opt.sc:
            self.mm_path = 'warped_thermal'
        ...
        # center pose
        self.poses, self.pts3d = center_poses(poses, pts3d, self.opt.enable_cam_center, self.opt.offset)
        ...
        if self.type != 'test':
            ...
            for i, k in enumerate(tqdm.tqdm(imkeys)):
                ...
                # camera near far
                self.cam_near_far.append([np.percentile(depth, 3), np.percentile(depth, 95)])
        ...
        if self.type == 'test':
            ...
            #generate novel views by interpolating between the known poses.

            poses = []
            fs = np.arange(0, len(self.poses))
            fs = fs[::4]
            n_test = 5
            ...
            for i in range(1, len(fs)):
                ...
                for j in range(n_test + 1):  # Changed variable name to 'j'
                    ratio = np.sin(((j / n_test) - 0.5) * np.pi) * 0.5 + 0.5
            ...
            for f in tqdm.tqdm(img_paths, desc=f'Loading {self.type} data'):
                ...
                if self.opt.ts:
                    ...
                    image = cv2.imread(f, cv2.IMREAD_COLOR) # [H, W, 3] o [H, W, 4]
                    ...
                    if self.opt.rgbt or self.opt.sc:
                        fn = f.split('/')[-1][:-4]+'.npy'
                        f2 = os.path.join(self.root_path,self.mm_path,fn)
                        image2 = np.load(f2)
                        
                        self.temp_images.append(image2)

            self.images = np.stack(self.images, axis=0)
            if self.opt.rgbt or self.opt.sc:
                self.temp_images = np.stack(self.temp_images,axis=0)
        ...
        if self.images is not None:
            if self.opt.ts:
                self.images = torch.from_numpy(np.stack(self.images, axis=0).astype(np.float32)) # [N, H, W, C]
            else:
                self.images = torch.from_numpy(np.stack(self.images, axis=0).astype(np.uint8)) # [N, H, W, C]
            if self.temp_images is not None and (self.opt.rgbt or self.opt.sc):
                self.temp_images = torch.from_numpy(np.stack(self.temp_images, axis=0).astype(np.float32)) # [N, H, W, C]
        ...
        if self.preload:
            ...
            if self.temp_images is not None and (self.opt.rgbt or self.opt.sc):
                self.temp_images = self.temp_images.to(self.device)
            if self.cam_near_far is not None:
    ...
    def collate(self, index):
        ...
        if not self.opt.preload and self.training:
            index = index.cpu()
        ...
        if self.images is not None:
            ...
            if self.training:
                ...
                if self.opt.ts:
                    images = self.images[index, rays['j'], rays['i']].float().to(self.device) / self.opt.tv # [N, 3/4]    
                else:
                    images = self.images[index, rays['j'], rays['i']].float().to(self.device) / 255 # [N, 3/4]
                    if self.opt.rgbt or self.opt.sc:
                        temp_images = self.temp_images[index, rays['j'], rays['i']].float().to(self.device)/self.opt.tv 
            else:
                
                if self.opt.ts:
                    images = self.images[index].squeeze(0).float().to(self.device) / self.opt.tv # [H, W, 3/4]
                else:
                    images = self.images[index].squeeze(0).float().to(self.device) / 255 # [H, W, 3/4]
                    if self.opt.rgbt or self.opt.sc:
                        temp_images = self.temp_images[index].squeeze(0).float().to(self.device)/self.opt.tv 
            if self.training:
                if self.opt.ts:
                    ...
                    if self.opt.rgbt or self.opt.sc:
                        temp_images = temp_images.view(-1)

            results['images'] = images
            if self.opt.rgbt or self.opt.sc:
                results['temp_images'] = temp_images
